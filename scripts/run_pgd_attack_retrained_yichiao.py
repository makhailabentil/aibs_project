...
def main() -> None:
    ...
    parser.add_argument("--ckpt", type=str, default=None, help="Path to fine-tuned checkpoint")
    ...
    try:
        ...
        if args.ckpt is not None:
            ...
            _log(f"Loaded fine-tuned backbone from: {ckpt_path}")
        else:
        ...
        _log(f"Failed to load checkpoint/model: {e}")
        sys.exit(1)
    ...
    if args.save_results:
        ...
        tag = "retrained" if args.ckpt is not None else "baseline"
        filename = f"pgd_{tag}_{args.mode}_eps{args.eps:.4f}_{timestamp}.json"
